...
create table if not exists public.agent_marketplace (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users(id) on delete cascade,
  name text not null,
  role text not null,
...
drop policy if exists "users manage own memories" on public.agent_memories;
...
drop policy if exists "users manage own world states" on public.world_states;
...
drop policy if exists "admin reads settings" on public.system_settings;
create policy "admin reads settings" on public.system_settings
  for select using ((auth.jwt() ->> 'email') = 'parreiracarvalhod@gmail.com');

drop policy if exists "admin writes settings" on public.system_settings;
...
drop policy if exists "anyone reads agent marketplace" on public.agent_marketplace;
create policy "anyone reads agent marketplace" on public.agent_marketplace
  for select using (true);

drop policy if exists "authenticated users post agents" on public.agent_marketplace;
create policy "authenticated users post agents" on public.agent_marketplace
  for insert with check (auth.uid() = user_id);

drop policy if exists "owners update own marketplace agents" on public.agent_marketplace;
create policy "owners update own marketplace agents" on public.agent_marketplace
