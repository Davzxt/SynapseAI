...
app.post('/api/create-checkout-session', async (req, res) => {
  ...
  if (!stripeKey) return res.status(501).json({ error: 'Configure STRIPE_SECRET_KEY no .env ou na Vercel.' });

  const origin = req.headers.origin || process.env.SITE_URL || `http://localhost:${port}`;
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) return res.status(response.status).json({ error: data.error?.message || 'Erro no Stripe.' });
    res.json({ url: data.url });
  } catch (error) {
    res.status(500).json({ error: error.message || 'Erro inesperado no checkout.' });
  }
});
